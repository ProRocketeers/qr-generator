  ...
  if (!(globalThis as any).document) {
    ;(globalThis as any).document = {
      createElement: (tag: string) =>
        tag === 'canvas' ? createCanvas(300, 300) : {},
    }
  }
  const c = createCanvas(1, 1)
  ;(globalThis as any).HTMLCanvasElement = c.constructor
  ;(globalThis as any).CanvasRenderingContext2D = c.getContext('2d').constructor
